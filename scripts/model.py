...
def main():
    ...
    estimated_infection_rate = solver(
        ...
        initial=initial,
        observed_infected=infected_data,
...
def objective(infection_rate, population, initial, observed_infected, recovery_rate):
    ...
    # Solve ODE
    compartments = odeint(
        sir_model, initial_values, t, args=(infection_rate, recovery_rate, population)
    )
...
def solver(func, population, initial, observed_infected, recovery_rate, start_date):
    ...
    solution = minimize(
        ...
        args=(population, initial, observed_infected, recovery_rate),
    )
    ...
    compartments = odeint(
        ...
        initial_values,
        t,
        args=(estimated_infection_rate, recovery_rate, population),
    )
    infected = compartments[:, 1]
    ...
    plt.title(
        r"Count vs. Time $(\beta={}, \gamma={}, R_0={}, r^2={})$".format(
            round(estimated_infection_rate, 4),
...
def project(population, initial, days, infection_rate, recovery_rate, start_date):
    ...
    ax.annotate(
        "{:,}".format(int(round(susceptible[-1]))),
        (t[-1] - 20, 10 ** (np.log10(susceptible[-1]) + 0.15)),
        ha="center",
    )
    t_max_infected = np.where(infected == max(infected))[0][0]
    plt.semilogy(t_max_infected, max(infected), "ro")
    ax.annotate(
        "{:,}".format(int(round(max(infected)))),
        (t_max_infected, 10 ** (np.log10(max(infected)) - 0.50)),
        ha="center",
    )
    plt.semilogy(t[-1], infected[-1], "ro")
    ax.annotate(
        "{:,}".format(int(round(infected[-1]))),
        (t[-1], 10 ** (np.log10(infected[-1]) + 0.2)),
        ha="center",
    )
    ...
    plt.ylabel("count")
    plt.title(
        r"Count vs. Time $(\beta={}, \gamma={}, R_0={})$".format(
...
def get_infected(url):
    ...
    us_state_lookups = pd.merge(
        us_state_codes, us_state_coordinates, on=["state"], how="inner"
    )
