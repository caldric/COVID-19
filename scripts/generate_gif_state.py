...
def main():
    ...
    colors = ["#fff0b0", "#edae4b", "#e3683c", "#bf2a48", "#6d2059", "#310958"]
    ...
    infected_data_at = {}
    for date in dates:
        infected_data_at[date] = infected_data.loc[
    ...
    for i in range(len(dates)):
        ...
        path = join(
            "images/state_timeline", "{}.png".format(dates[i].strftime("%Y-%m-%d"))
        )
        total_cases = sum(data["infected_count"])
        if i > 0:
            new_cases = total_cases - sum(
                infected_data_at[dates[i - 1]]["infected_count"]
            )
        else:
            new_cases = 0
        ...
        fig.update_layout(
            title_text="<b>COVID-19 US Infected</b><br>Confirmed cases as of "
            + dates[i].strftime("%B %d, %Y")
            + "<br>Total cases: "
            + "{:,}<br>New cases: {:,}".format(total_cases, new_cases),
            geo=dict(scope="usa"),
        )
...
def wrangle_data(url):
    ...
    infected_data = infected_data.melt(
        id_vars=["state"], var_name="date", value_name="infected_count"
    )
    ...
    infected_data = infected_data[
        ["state", "state_code", "latitude", "longitude", "date", "infected_count"]
    ]
